...
describe("offline database backup", () => {
  ...
  it("round-trips all device-local records through a JSON blob", async () => {
    ...
    await db.homeworkReports.put({
      ...
      items: [
        { subjectId: "subject-1", details: "Complete exercise 4.", order: 0 },
      ],
      createdAt: "2026-08-15T00:00:00.000Z",
      updatedAt: "2026-08-15T00:00:00.000Z",
    });
    ...
    expect((await db.settings.get("school"))?.schoolName).toBe(
      "Offline School",
    );
    expect((await db.students.get("student-1"))?.phone).toBe("+92 300 0000000");
    expect((await db.students.get("student-1"))?.dateOfBirth).toBe(
      "2014-05-12",
    );
    expect((await db.students.get("student-1"))?.photoDataUrl).toContain(
      "data:image/png",
    );
    expect(
      (await db.attendance.get("register-1:enrollment-1:3:1"))?.status,
    ).toBe("P");
    expect((await db.feeEntries.get("register-1:enrollment-1:1"))?.ftf).toBe(
      10000,
    );
    expect((await db.subjects.get("subject-1"))?.name).toBe("Mathematics");
    expect((await db.tests.get("test-1"))?.totalMarks).toBe(40);
    expect((await db.testRoster.get("test-1:enrollment-1"))?.enrollmentId).toBe(
      "enrollment-1",
    );
