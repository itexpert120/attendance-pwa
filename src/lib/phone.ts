...
export function normalizePhoneNumber(value: string) {
  ...
  throw new Error('Enter a valid phone number, for example 0300 1234567 or +92 300 1234567.')
}

export function displayPhoneNumber(value: string) {
  if (!value.trim()) return ''
  try {
    return parsePhoneNumber(value, DEFAULT_COUNTRY)?.formatInternational() ?? value
  } catch {
    return value
  }
}
