...
export function daysForRegister(register: Register) {
  return Array.from(
    { length: daysInMonth(register.year, register.month) },
    (_, index) => index + 1,
  );
}
...
export function isEnrollmentActiveOn(enrollment: Enrollment, date: string) {
  return (
    enrollment.admittedOn <= date &&
    (!enrollment.struckOffOn || enrollment.struckOffOn >= date)
  );
}

export function isEnrollmentInMonth(
  ...
  const end = dateKey(
    register.year,
    register.month,
    daysInMonth(register.year, register.month),
  );
  return (
    enrollment.admittedOn <= end &&
    (!enrollment.struckOffOn || enrollment.struckOffOn >= start)
  );
}
  ...
  return enrollments
    .filter(
      (enrollment) =>
        ...
        (!register || isEnrollmentInMonth(enrollment, register)),
    )
    .map((enrollment) => ({
      enrollment,
...
export function academicYearStartYear(
  ...
  return register.month >= settings.academicYearStartMonth
    ? register.year
    : register.year - 1;
}

export function academicYearStartYearForDate(
  date: string,
  settings: SchoolSettings,
) {
...
export function isRegisterEarlierInAcademicYear(
  ...
  return (
    candidate.year * 12 + candidate.month < current.year * 12 + current.month
  );
}

export function customHolidayForDay(
  holidays: Holiday[],
  registerId: string,
  day: number,
) {
  return holidays.find(
    (holiday) => holiday.registerId === registerId && holiday.day === day,
  );
}
...
export function isHolidayDay(
  ...
  return (
    isWeekend(register.year, register.month, day) ||
    Boolean(customHolidayForDay(holidays, register.id, day))
  );
}
...
export function broughtForwardAttendance(
  ...
  const earlierRegisterIds = new Set(
    registers
      .filter((register) =>
        isRegisterEarlierInAcademicYear(register, current, settings),
      )
...
export function workingTimings(register: Register, holidays: Holiday[]) {
  return (
    daysForRegister(register).filter(
      (day) => !isHolidayDay(holidays, register, day),
    ).length * 2
  );
}
...
export function monthlyMovement(rows: EnrollmentRow[], register: Register) {
  ...
  return {
    beginning: rows.filter((row) => isEnrollmentActiveOn(row.enrollment, start))
      .length,
    end: rows.filter((row) => isEnrollmentActiveOn(row.enrollment, end)).length,
    admitted: rows.filter(
      (row) =>
        row.enrollment.admittedOn >= start && row.enrollment.admittedOn <= end,
    ).length,
...
export function attendanceReport(
  ...
  const marksByCell = new Map(
    marks
      ...
      .map((mark) => [
        `${mark.enrollmentId}:${mark.day}:${mark.session}`,
        mark.status,
      ]),
  );
  const students = rows.map((row) => {
    ...
    for (let day = period.startDay; day <= period.endDay; day += 1) {
      ...
      if (
        isHolidayDay(holidays, register, day) ||
        !isEnrollmentActiveOn(row.enrollment, date)
      ) {
        continue;
      }
      for (const session of [1, 2] as const) {
        const status = marksByCell.get(
          `${row.enrollment.id}:${day}:${session}`,
        );
...
export function feesForStudent(
  ...
  return addFees(
    ...entries.filter(
      (entry) =>
        entry.registerId === registerId && entry.enrollmentId === enrollmentId,
    ),
  );
}
...
export function feesForInstallment(
  ...
  return addFees(
    ...entries.filter(
      (entry) =>
        entry.registerId === registerId && entry.installment === installment,
    ),
  );
}
...
export function testSummary(
  ...
  const absentCount = testResults.filter(
  ...
  return {
    ...
    average: numericMarks.length
      ? numericMarks.reduce((total, marks) => total + marks, 0) /
        numericMarks.length
      : null,
...
export function subjectReportPeriod(
  type: SubjectReportPeriodType,
  anchorDate: string,
) {
...
export function testReportAggregate(
  ...
  const relevantResults = results.filter(
    (result) =>
      ...
      rosterKeys.has(`${result.testId}:${result.enrollmentId}`),
  );
  ...
  const rosterCount = roster.filter((entry) =>
    testIds.has(entry.testId),
  ).length;

  return {
    totalTests: tests.length,
    numericCount: percentages.length,
    absentCount: relevantResults.filter((result) => result.status === "absent")
      .length,
